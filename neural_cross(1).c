...
float *hidden_layer_1,output_layer[10],err = 0,err2=0,delta2[10];
float **weights_1,**weights_2,**DEL_2,**DEL_1,lr = 0.01;
float y[10];
...
float calcost2(float A[10],float B[10]){
	float c = 0;
	int i;
	for( i=0;i<10;i++)
		c -= B[i]*(log(A[i])) + (1-B[i])*(log(1-A[i]));
	return c;
}
void calc_error(float *pred,int out,int train){
	...
	for (i = 0; i <=no_hid_units; ++i){
		for (j = 0; j < 10; ++j){
			//printf("%f ",weights_2[i][j]);
			weights_2[i][j] -= (1.0/train_size)*lr*DEL_2[i][j];
			//printf("%f \n",weights_2[i][j]);
		}
		//printf("\n");
	}
	...
	for(i = 0;i<=16;i++){
		...
		for (j = 0; j <=no_hid_units; ++j){
			weights_1[i][j] -= (1.0/train_size)*lr*DEL_1[i][j];
			////printf("%f ",weights_1[i][j]);
		}
		//printf("\n");
	}
...
int stopingCondition(float ep)
{
	...
	if(count1>(17*(no_hid_units+1) + 10*(no_hid_units+1)))
		return 1;
	return 0;
}
int main(){
